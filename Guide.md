...
`npm run test` - команда для тестовой сборки проекта. Разметка форматируется. CSS, JS и изображения не минифицируются. Сервера `browser-sync` не запускается.
...
Чтобы подключить сторонние js-файлы (библиотеки) просто скопируйте их в папку __src/vendor__ в папку, соответствующую библиотеке, либо, если планируется подключение из папки __src/js/**__, подключите через import в файл __main.js__ . Сборка соберет все js файлы библиотек, сделает из них общий бандл и подключит в проект.
